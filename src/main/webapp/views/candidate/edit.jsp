        ...
        <div class="col-lg-12 col-md-12 col-sm-12">
            <ul class="menu">
                <li class="item-left"><a class="btn btn-default"
                                         href="<c:url value="/image?id=${candidate.id}&delete=1"/>">Удалить фото</a>
                </li>
                <li class="item-left"><a class="btn btn-default"
                                         href="<c:url value="/image?id=${candidate.id}&download=1"/>">Скачать фото</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <form action="<c:url value="/candidates.do"/>" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label>Ф.И.О.</label>
                    <input id="name" name="nName" type="text" class="form-control"
                           value="<c:out value="${candidate.name}"/>" title="Заполните имя кандидата!" required/>
                    <label>Претендует на позицию</label>
                    <input id="position" name="nPosition" type="text" class="form-control"
                           value="<c:out value="${candidate.position}"/>"
                           title="Заполните позицию, на которую претендует кандидат!" required/>
                    <label>Город</label>
                    <select id="cityList" class="form-control">
                        <option id="null" selected="1">Нет города</option>
                    </select>
                    <input type="hidden" name="nSelectedCityId" id="selectedCityId"
                           value="<c:out value="${candidate.cityId}"/>"/>
                </div>
                <div class="form-group">
                    <label>Фото</label>
